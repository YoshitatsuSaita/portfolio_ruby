...
私自身が趣味でDiscord上での句会に参加しており、毎回第三者からのメンションでのお題の通達・匿名性を保つため、第三者へのメンションを介しての句の収集・提出状況確認・点数集計に多くの時間がかかっていました。
参加者が増えるほど管理の手間も増し、主催者・参加者双方の負担になっていたため、これを仕組み化して解決することを目的に制作しました。

## 解説動画

https://youtu.be/pe3s0FO7BYc

## 本番環境

https://haiku-app-uzca.onrender.com
※ 無料プランのため、初回アクセスに1〜2分かかる場合があります。
...
- ゲストログイン (一般ユーザー / 管理者ユーザー)
- パスワードリセット (メールによるリセットリンク送信、2時間の有効期限付き)
- プロフィール編集 (自己紹介文: 500文字以内)
...
- 添削案・添削理由の記入
- 評価の編集・削除（モーダルから操作）
- バリデーションエラー時のモーダル内エラー表示 (Turbo Stream による非同期レスポンス、入力内容を保持)
- 1俳句につき1ユーザー1評価の制約
- 自分の俳句への評価不可
...
| `GMAIL_USERNAME` | Gmail アドレス (パスワードリセットメール送信に必要) |
| `GMAIL_PASSWORD` | Gmail アプリパスワード (パスワードリセットメール送信に必要) |
